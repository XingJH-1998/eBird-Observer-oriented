...
{
#Figs2
ob.num <- readRDS("/Users/tangerine/Documents/Lab_data/ebird_covid/observation_num.rda")
ob.num.re <- ob.num %>% tidyr::spread(key = Nation.status, value = N_observation)
ob.num.re$International <- ob.num.re$International+ob.num.re$`<NA>`
ob.num.re <- ob.num.re[,-5]
ob.num.re$date <- paste0(ob.num.re$Year, "-", ob.num.re$Month, "-01")
ob.num.re$date <- as.Date(ob.num.re$date)

ggplot(ob.num.re, aes(x=date))+
  geom_line(aes(y=Local, color="Local"))+
  geom_line(aes(y=International*5, color="International"))+
  scale_y_continuous(name = "Number of local observation", sec.axis = sec_axis(~./5,labels = scales::label_number(big.mark = ","), name= "Number of international observation"),labels = scales::label_number(big.mark = ","))+
  theme(axis.title.y.right =element_text(angle=90))+
  xlab("Date")+
  scale_color_manual(values = c("Local"="steelblue", "International"="indianred"))+
  labs(title = "Number of observation", color="Observation type")+
  theme_bw()+
  theme(
    legend.position = c(.05,.75),
    legend.justification = c(0,0),
...
ggsave("FigS2.png", height = 6, width = 10, dpi = 300)
ggsave("FigS2.pdf", height = 6, width = 10, dpi = 300)
}
...
p3 <- ggplot(data.se.fig4.2, aes(x=date))+
...
##Fig2 combine
library(cowplot)
p13 <- plot_grid(p1,p3, align="v", ncol=1)
p.fig2 <- ggpubr::ggarrange(p13,p2,nrow = 1)
p.fig2 ##12*8
ggsave("Fig2.png", width = 18, height = 10, dpi = 300)
ggsave("Fig2.pdf", width = 18, height = 10, dpi = 300)
...
#Fig3a
p1.old <- ggplot(data.1.se %>% filter(continent=="South-eastern Asia"))+
...
p1 <- ggplot(data.1.se %>% filter(continent=="South-eastern Asia"))+
  geom_bar(stat = "identity", position = "stack", aes(y=COUNTRY, x=Nsum, fill=Nation.status))+
  facet_wrap(~Year, ncol = 5)+
  ggtitle("(a) South-eastern Asia")+
  xlab(NULL)+
  ylab(NULL)+
...
#Fig3b
p2.old <- ggplot(data.1.se %>% filter(continent=="Western Europe") %>% filter(COUNTRY!="Monaco"))+ ###Monaca only have international observations, which is an outlier
  geom_bar(stat = "identity", position = "fill", aes(y=COUNTRY, x=Nsum, fill=Nation.status))+
  facet_wrap(~Year, ncol = 5)+
...
data.1 <- data
data.1.se <- data.1[,.(Nsum=sum(N.sum)), by=list(COUNTRY, Nation.status,Year)]
data.1.se$Code <- countrycode(data.1.se$COUNTRY, 'country.name', 'iso2c')
data.1.se$Continent <- countrycode(data.1.se$COUNTRY, 'country.name', 'continent')
data.1.se$region <- countrycode(data.1.se$COUNTRY, 'country.name', 'region')
data.1.se$subregion <- countrycode(data.1.se$COUNTRY, 'country.name', 'un.regionsub.name')
View(country_info)

world_map <- getMap()
centroids <- data.frame(Code = world_map$ISO_A2, 
                        longitude = coordinates(world_map)[, 1], 
                        latitude = coordinates(world_map)[, 2])
centroids$Country <- rownames(centroids)

data.1.se <- merge(data.1.se, centroids, by.x="Code", by.y="Code", all.x=TRUE)
data.1.se[is.na(data.1.se$longitude),c(1)] %>% unique()

data.filter <- data.1.se %>% group_by(COUNTRY) %>% filter(n() ==10)
data.filter[is.na(data.filter$longitude),c(1)] %>% unique()
data.filter <- tidyr::spread(data.filter, Nation.status, Nsum)
data.filter <- data.filter %>% mutate(Sum=International+Local)
data.filter <- data.filter %>% mutate(propInter =International/Sum)
table(data.filter$Continent)
table(data.filter$region)
table(data.filter$subregion)
data.filter <- as.data.table(data.filter)
data.filter.se <- data.filter[,.(Sumyear=sum(Sum)), by=list(COUNTRY, Continent, region, subregion)]
data.filter.se <- data.filter.se %>% arrange(Continent, region, subregion, Sumyear)
africa.se <- data.filter.se %>% filter(Continent=="Africa") %>% filter(region=="Sub-Saharan Africa") %>% arrange(Sumyear) %>% tail(10)
europe.se <- data.filter.se %>% filter(Continent=="Europe") %>% arrange(Sumyear) %>% tail(10)


p1 <- ggplot(data.filter %>% filter(COUNTRY %in% africa.se$COUNTRY), aes(x = Year, y = COUNTRY, size = Sum, fill = propInter)) +
  geom_point(alpha = 0.7, shape = 21) +
  scale_size_continuous(range = c(3, 30)) +
  scale_fill_gradient(low = "indianred", high = "steelblue") +
  theme_bw() +
  labs(fill = "Proportion of International Observation") +
  theme(legend.position = "bottom")+
  xlab(NULL)+
  ylab(NULL)+
  labs(size="Number of observation", title = "Sub-Saharan Africa")+
...
p2 <- ggplot(data.filter %>% filter(COUNTRY %in% europe.se$COUNTRY), aes(x = Year, y = COUNTRY, size = Sum, fill = propInter)) +
  geom_point(alpha = 0.7, shape = 21) +
  scale_size_continuous(range = c(3, 30)) +
  scale_fill_gradient(low = "indianred", high = "steelblue") +
  theme_bw() +
  ...
  xlab(NULL)+
  ylab(NULL)+
  labs(size="Number of observation", title = "Europe")+
  coord_fixed(ratio = 0.25)+
  guides(size = guide_legend(direction = "vertical", 
                             title.position = "top", 
                             label.position = "right", 
...
p <- ggpubr::ggarrange(p1, p2, align = "v", ncol=1, common.legend = TRUE, legend = "right")
p
#15*20


ggplot(data.filter %>% filter(subregion=="Western Europe"), aes(x = Year, y = COUNTRY, size = Sum, fill = propInter)) +
  geom_point(alpha = 0.7, shape = 21) +
  scale_size_continuous(range = c(3, 30)) +
  scale_fill_gradient(low = "indianred", high = "steelblue", space = 1) +
  theme_minimal() +
  labs(fill = "Proportion of International") +
  theme(legend.position = "bottom")
...
data.18 <- data.filter %>% filter(Year==2018)
#data.18 <- tidyr::spread(data = data.18, key = Nation.status, value=Nsum)
#data.18$Sum <- data.18$International+ data.18$Local
data.18$end1 <- with(data.18, International / (Local + International) * 2 * pi)
data.18$end2 <- 2*pi
data.18$scale.sum <- normalize(log(data.18$Sum))
summary(data.18$scale.sum)


ggplot(data.18 %>% filter(region=="Europe & Central Asia")) + 
  borders("world", colour = "black", fill = "azure") +
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum, start = 0, end = end1, fill = "International"))+
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum, start = end1, end = end2, fill = "Local"))+
  coord_fixed() +
  scale_fill_manual(values = c("International" = "steelblue", "Local" = "indianred")) +
  theme_bw()+
  xlab(NULL)+
  ylab(NULL)+
  labs(fill="Type", title = "Europe 2018")+
  theme(
...
ggplot(data.18 %>% filter(subregion=="South-eastern Asia")) + 
  borders("world", colour = "black", fill = "azure") +
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum, start = 0, end = end1, fill = "International"))+
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum, start = end1, end = end2, fill = "Local"))+
  coord_fixed() +
  ...
  xlab(NULL)+
  ylab(NULL)+
  labs(fill="Type", title = "Southeast Asia 2018")+
  theme(
    axis.text.x = element_blank(),
    axis.text.y = element_blank()
  )+
  coord_cartesian(xlim = c(90, 140), 
                  ylim = c(-11, 20))


ggplot(data.18 %>% filter(Continent=="Africa")) + 
  borders("world", colour = "black", fill = "azure") +
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum, start = 0, end = end1, fill = "International"))+
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum, start = end1, end = end2, fill = "Local"))+
  coord_fixed() +
  ...
  xlab(NULL)+
  ylab(NULL)+
  labs(fill="Type", title = "Africa 2018")+
  theme(
    axis.text.x = element_blank(),
    axis.text.y = element_blank()
  )+
  coord_cartesian(xlim = c(-18, 51), 
                  ylim = c(-35, 37))

data.21 <- data.filter %>% filter(Year==2021)
data.21 <- tidyr::spread(data = data.21, key = Nation.status, value=Nsum)
data.21$Sum <- data.21$International+ data.21$Local
data.21$end1 <- with(data.21, International / (Local + International) * 2 * pi)
data.21$end2 <- 2*pi
data.21$scale.sum <- scale(log(data.21$Sum))
summary(data.21$scale.sum)

ggplot(data.21) + 
  borders("world", colour = "black", fill = "azure") +
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum+3, start = 0, end = end1, fill = "International"))+
  geom_arc_bar(aes(x0 = longitude, y0 = latitude, r0 = 0, r = scale.sum+3, start = end1, end = end2, fill = "Local"))+
  coord_fixed() +
  scale_fill_manual(values = c("International" = "steelblue", "Local" = "indianred")) +
  theme_minimal()
}
...
p.fig3
ggsave("Fig4.png", width = 10, height = 18, dpi = 300)
ggsave("Fig4.pdf", width = 10, height = 18, dpi = 300)
#18*10


###Fig5
data$N_species_eBird[is.na(data$N_species_eBird)] <- 0
data$difference <- (data$N_species_eBird-data$N_species_IUCN)/data$N_species_IUCN
...
{
pfig5 <- ggplot()+
  geom_tile(data=data %>% mutate(difference = ifelse(difference>=1, 1, difference)), aes(x=LONGITUDE, y=LATITUDE, fill= difference))+
  scale_fill_gradient2(low = "white", mid = "blue", high = "red", midpoint = 0, na.value = "grey", space=1, breaks=c(-1,-.5,0,.5,1) , labels=c("-1","-0.5","0","0.5",">1"))+
  geom_polygon(data=map %>% filter(long<=179), aes(x=long, y=lat, group=group), color="Black", linewidth=.1, fill=NA)+
  coord_equal()+
  theme_minimal()+
  labs(fill=NULL)+
  xlab(NULL)+
  ylab(NULL)+
  labs(title = "Sampling adequacy of eBird")+
  theme_bw()
pfig5
...
grid <- data[,c(1,2)] %>% as.data.frame()
grid <- grid[,-3]
rst <- raster("/Users/tangerine/Desktop/bio1.tiff")
plot(rst)
grid <- SpatialPoints(grid)
proj4string(grid) <- crs(rst)
values_at_points <- extract(rst, grid)
data_filter <- data[!is.na(values_at_points),]

pfig5 <- ggplot()+
  geom_tile(data=data_filter %>% mutate(difference = ifelse(difference>=1, 1, difference)), aes(x=LONGITUDE, y=LATITUDE, fill= difference))+
  scale_fill_gradient2(low = "steelblue", mid = "white", high = "indianred", midpoint = 0, na.value = "grey", space=1, breaks=c(-1,-.5,0,.5,1) , labels=c("-1","-0.5","0","0.5",">1"))+
  geom_polygon(data=map %>% filter(long<=179 & lat>=-60), aes(x=long, y=lat, group=group), color="Black", linewidth=.1, fill=NA)+
  coord_equal()+
  theme_minimal()+
  labs(fill=NULL)+
  xlab(NULL)+
